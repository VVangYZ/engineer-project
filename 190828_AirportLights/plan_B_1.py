...
filename = 'input_1_1'
node_file = 'node_B.csv'
elem_file = 'elem_B.csv'

wind_beam = [419.8199238646487, 157.43247144924322]
wind_pier = [1286.2651839410798, 1159.2393375795968, 635.3582272076605, 945.7066198390747, 635.3582272076605]
...
foot_path_load = 1.5 + 0.2 + 0.3                # 检修道荷载，单位 kN/m
...
for i, j in elem_beam.iterrows():
    if node_beam.loc[j['n1']]['y'] < 100:
        ...
        if j['WHAT'] in ['upper chord', ]:
            wind_f = wind_beam[0] * 2 /1000
        elif j['WHAT'] in ['lower chord', ]:
            wind_f = wind_beam[0] /1000
        elif j['WHAT'] in ['web member V', 'web member D']:
            wind_f = wind_beam[1] /1000
        else:
            continue
