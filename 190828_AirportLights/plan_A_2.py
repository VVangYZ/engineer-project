...
filename = 'new_3'
node_file = 'newnode_1.csv'
node_x_file = 'newnode_x_1.csv'
elem_file = 'newelem_1.csv'
...
sec_pier = {'pier1':[0.5, 0.015], 'pier2': [0.3, 0.006], 'pier3': [0.4, 0.008], 'pier4': [0.4, 0.006]}
...
## 主梁参数
spans = [30, 75, 80, 80, 80, 70, 0]                  # 各跨度长度（默认两侧为悬臂，无悬臂则输入 0）
fixed = 6                  # 第几个支座为梁上支座
beam_start = -2.5
...
## 荷载信息
light_loc = np.linspace(0, 360 + 30, 13 + 1)                   # 助航灯位置
light_weight = 20                   # 单个助航灯重量，单位 kN
foot_path_load = 2                 # 检修道荷载，单位 kN/m

# 覆冰荷载试算
ice_beam = [18515] * 2 + [9750] * 2 + [6750] * 4
ice_pier = [24268, 14844, 19556, 19556]
ice_beam = np.array(ice_beam) / 1000000 * 10
ice_pier = np.array(ice_pier) / 1000000 * 10
...
for i in range(len(spans)-2):
    ...
    pier_bot1 = pier_loc1 * np.tan(beam_angle) - pier_H[i]
    pier_bot2 = pier_loc2 * np.tan(beam_angle) - pier_H[i]
...
for i, j in elem_pier.iterrows():
    if j['WHAT'] == 'pier-1' or j['WHAT'] == 'pier-2':
...
for i, j in elem_beam.iterrows():
    if node_beam.loc[j['n1']]['y'] < 100:
        ...
        if j['WHAT'] in ['upper chord', ]:
            ...
            continue
        L = np.linalg.norm(node_beam.loc[j['n1']][0:3] - node_beam.loc[j['n2']][0:3])
        F = wind_f * L
        mct_file.write('%d, 0, %f, 0, 0, 0, 0, %s\n'%(j['n1'], F/2, 'wind-t'))
        mct_file.write('%d, 0, %f, 0, 0, 0, 0, %s\n'%(j['n2'], F/2, 'wind-t'))
...
for i, j in elem_pier.iterrows():
    if node.loc[j['n1']]['y'] < 0:
        ...
        if j['WHAT'] in ['pier-1', 'pier-2', 'pier-4']:
            ...
            continue
        L = np.linalg.norm(node.loc[j['n1']][0:3] - node.loc[j['n2']][0:3])
        F = wind_f * L
        mct_file.write('%d, 0, %f, 0, 0, 0, 0, %s\n'%(j['n1'], F/2, 'wind-t'))
        mct_file.write('%d, 0, %f, 0, 0, 0, 0, %s\n'%(j['n2'], F/2, 'wind-t'))
...
### 覆冰荷载
mct_file.write('*USE-STLD, ICE\n')
mct_file.write('*CONLOAD\n')
for i, j in elem_beam.iterrows():
    ice_f = ice_beam[j['s'] - 1]
    L = np.linalg.norm(node_beam.loc[j['n1']][0:3] - node_beam.loc[j['n2']][0:3])
    F = ice_f * L
    mct_file.write('%d, 0, 0, %f, 0, 0, 0, %s\n'%(j['n1'], -F/2, 'ice'))
    mct_file.write('%d, 0, 0, %f, 0, 0, 0, %s\n'%(j['n2'], -F/2, 'ice'))
for i, j in elem_pier.iterrows():
    ice_f = ice_pier[int(j['WHAT'][-1]) - 1]
    L = np.linalg.norm(node.loc[j['n1']][0:3] - node.loc[j['n2']][0:3])
    F = ice_f * L
    mct_file.write('%d, 0, 0, %f, 0, 0, 0, %s\n'%(j['n1'], -F/2, 'ice'))
    mct_file.write('%d, 0, 0, %f, 0, 0, 0, %s\n'%(j['n2'], -F/2, 'ice'))
...
### 温度荷载
mct_file.write('*USE-STLD, TEMP_UP\n')
mct_file.write('*ELTEMPER\n')
for i, j in elem.iterrows():
    mct_file.write('%d, 15, temp_up\n'%(i))

mct_file.write('*USE-STLD, TEMP_DOWN\n')
mct_file.write('*ELTEMPER\n')
for i, j in elem.iterrows():
    mct_file.write('%d, -15, temp_down\n'%(i))

### 荷载组合
mct_file.write('*LOADCOMB\n')
mct_file.write('NAME=w1, GEN, ACTIVE, 0, 0, , 0, 0\n')
mct_file.write('ST, DEAD, 1\n')
mct_file.write('NAME=w12, GEN, ACTIVE, 0, 0, , 0, 0\n')
mct_file.write('ST, DEAD, 1, ST, SUPER-DEAD, 1\n')
mct_file.write('NAME=w123, GEN, ACTIVE, 0, 0, , 0, 0\n')
mct_file.write('ST, DEAD, 1, ST, SUPER-DEAD, 1, ST, ICE, 1\n')
mct_file.write('NAME=temp, GEN, ACTIVE, 0, 1, , 0, 0\n')
mct_file.write('ST, TEMP_UP, 1, ST, TEMP_DOWN, 1\n')
mct_file.write('NAME=com1, GEN, ACTIVE, 0, 0, , 0, 0\n')
mct_file.write('ST, DEAD, 1.2, ST, SUPER-DEAD, 1.2, ST, WIND-T, 1.4, CB, temp, 1.05\n')
mct_file.write('ST, ICE, 1.05\n')
...
for i, j in elem_pier.iterrows():
    if j['WHAT'] == 'pier-3':
        nnnum += 1
    if j['WHAT'] == 'pier-3' and nnnum %2 == 1:
        n11 = j['n1']
        n22 = np.where(node_pier.index == j['n1'])[0][0] - 4
        n22 = node_pier.index[n22]
        n21 = j['n2']
        n12 = np.where(node_pier.index == j['n2'])[0][0] - 4
        n12 = node_pier.index[n12]
        node_x.append([n11, n12])
        node_x.append([n21, n22])
        
node_x = pd.DataFrame(node_x)
node_x
node_x.to_csv('IO\\%s'%node_x_file)
