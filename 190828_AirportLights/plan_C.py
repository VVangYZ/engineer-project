...
filename = 'input_C'
node_file = 'node_C.csv'
elem_file = 'elem_C.csv'
    ...
    if (i - 1) * beam_il in beam_high:
        for j in [0, beam_high[(i - 1) * beam_il]]:
            if j == 0:
                ...
                num += 1
                node_beam.loc[num]  = [i, 0, j + i * np.tan(beam_angle), 1]
    else:
        for j in nz:
            if j == 0:
                for k in ny:
...
### 上弦杆
for i, j in node_beam.iterrows():
    beam_x = j['x']
    if j['z'] ==  j['x'] * np.tan(beam_angle) and j['x'] < beam_end:
        enum += 1
        elem_beam.loc[enum] = [mat['Q345'], sec_beam['uc'], 0,i, i + 3, 'upper chord']

### 下弦杆
for i, j in node_beam.iterrows():
    if j['z'] <  j['x'] * np.tan(beam_angle) and j['x'] < beam_end:
        enum += 1
        elem_beam.loc[enum] = [mat['Q345'], sec_beam['lc'], 0,i, i + 3, 'lower chord']

### 腹杆
#### 直腹杆
for i, j in node_beam.iterrows():
    if j['z'] <  j['x'] * np.tan(beam_angle):
        enum += 1
        elem_beam.loc[enum] = [mat['Q345'], sec_beam['wmv'], 0, i, i - 2, 'web member V']
        enum += 1
        elem_beam.loc[enum] = [mat['Q345'], sec_beam['wmv'], 0, i, i - 1, 'web member V']

#### 斜腹杆
for i, j in node_beam.iterrows():
    beam_oe = ((j['x'] - beam_start) // beam_il) % 2
    if beam_oe == 0 and j['z'] <  j['x'] * np.tan(beam_angle):
        if j['x'] != beam_start:
            enum += 1
            elem_beam.loc[enum] = [mat['Q345'], sec_beam['wmd'], 0, i, i - 5, 'web member D']
            enum += 1
            elem_beam.loc[enum] = [mat['Q345'], sec_beam['wmd'], 0, i, i - 4, 'web member D']
        if j['x'] != beam_end: 
            enum += 1
            elem_beam.loc[enum] = [mat['Q345'], sec_beam['wmd'], 0, i, i + 1, 'web member D']
            enum += 1
            elem_beam.loc[enum] = [mat['Q345'], sec_beam['wmd'], 0, i, i + 2, 'web member D']

### 横向联系
for i, j in node_beam.iterrows():
    if j['y'] < 0:
        enum += 1
        elem_beam.loc[enum] = [mat['Q345'], sec_beam['ulbt'], 0, i, i + 1, 'upper lateral bracing T']
        if j['x'] != beam_end:
            enum += 1
            elem_beam.loc[enum] = [mat['Q345'], sec_beam['ulbd'], 0, i, i + 4, 'upper lateral bracing D']
    elif j['y'] > 0:
        if j['x'] != beam_end:
            enum += 1
            elem_beam.loc[enum] = [mat['Q345'], sec_beam['ulbd'], 0, i, i + 2, 'upper lateral bracing D']
