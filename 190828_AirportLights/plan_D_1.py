...
filename = 'input_D_1'
node_file = 'node_D_1.csv'
elem_file = 'elem_D_1.csv'
...
## 主梁参数
spans = [92.5, 90, 90, 92.5, 0]                  # 各跨度长度（默认两侧为悬臂，无悬臂则输入 0）
beam_l = sum(spans)
...
for i, j in pier.iloc[[0, -1], :].iterrows():
    ...
    tower_top = tower_bot  + 24.5
    for k in np.linspace(tower_bot, tower_top - 5, 16)[:-1]:
        num += 1
